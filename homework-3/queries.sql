...
select pr.product_name, pr.units_in_stock, su.contact_name, su.phone
from products pr
	join suppliers su using (supplier_id)
	join categories ca using (category_id)
where pr.discontinued = 0 and pr.units_in_stock < 25 and ca.category_name in ('Dairy Products', 'Condiments')
order by pr.units_in_stock

-- 3. Список компаний заказчиков (company_name из табл customers), не сделавших ни одного заказа
select cu.company_name
from customers cu
	left join orders ord using (customer_id)
